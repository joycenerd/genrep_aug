...
def parse_option():
    ...
    parser.add_argument('--encoding_type', type=str, default='crossentropy',
...
def set_loader(opt):
    ...
    if opt.dataset == 'cifar10':
        ...
        if opt.mix_ratio == 0:
            train_dataset = datasets.ImageFolder(root=os.path.join(opt.data_folder, 'gen_train1300'),
                                        transform=TwoCropTransform(train_transform))
        else:
            train_dataset = MixDataset(root_dir=os.path.join(opt.data_folder, 'gen_train1300'), mix_ratio=opt.mix_ratio,
                                       transform=TwoCropTransform(train_transform))
        
    elif opt.dataset == 'gan':
        train_dataset = GansetDataset(root_dir=os.path.join(opt.data_folder, 'gen_train1300'), 
